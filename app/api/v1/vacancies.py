...
async def list_vacancies_endpoint(
    ...
    vacancies = await list_vacancies(session, timetable_mode_name, city)
...
async def create_vacancy_endpoint(
    ...
    vacancies = await create_vacancy(session, payload)
    return VacancyRead.model_validate(vacancies)
...
async def update_vacancy_endpoint(
    ...
    if not vacancy:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not found")
    updated_vacancy = await update_vacancy(session, vacancy, payload)
